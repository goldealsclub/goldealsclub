...
const DealFilters = ({ sourceDeals, children, defaultSort = "relevance" }: DealFiltersProps) => {
  ...
  // Brands sorted by hype/popularity (Nike, Adidas… first), volume as tie-breaker
  const allBrands = useMemo(() => {
    const counts: Record<string, number> = {};
    allDealsGlobal.forEach((d) => { counts[d.brand] = (counts[d.brand] || 0) + 1; });
    return sortBrandsByPopularity(Object.keys(counts), counts);
  ...
  const filtered = useMemo(() => {
    ...
    const getScore = (d: Deal): number => {
      ...
      // Partner boost (Snipes only) + top sneakers boost
      const src = d.source?.toLowerCase() || "";
      const merchant = d.merchant?.toLowerCase() || "";
      if (src === "snipes" || merchant.includes("snipes")) score += 20;
      if (d.category === "sneakers" && (d.discount_percent ?? 0) >= 30) score += 12;
