...
const AdminDashboard = () => {
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { key: "partners", label: "Partenaires", icon: <ShoppingBag className="w-4 h-4" /> },
      ...
      <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-12">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-2">
          ...
          <div className="flex items-center gap-2 -mx-3 sm:mx-0 px-3 sm:px-0 overflow-x-auto scrollbar-none">
            ...
            >
              🎬 <span className="hidden sm:inline">Vidéo du jour</span><span className="sm:hidden">Vidéo</span>
            </a>
          </div>
        </div>
        ...
        <div className="flex gap-0.5 sm:gap-1 mb-6 sm:mb-10 border-b border-foreground/8 overflow-x-auto scrollbar-none">
          {tabs.map((t) => (
            ...
            >
              ...
              <span className="xs:hidden">{t.key === "overview" ? "Vue" : t.key === "analytics" ? "Stats" : t.key === "partners" ? "Part." : t.key === "awin" ? "Awin" : t.key === "audit" ? "Audit" : "Users"}</span>
            </button>
          ))}
        </div>
